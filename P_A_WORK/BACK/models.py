...
class User(Base):
    ...
    tweets: Mapped[List["Tweet"]] = relationship(lazy="selectin", back_populates="author")
    ...
    async def get_by_api_key(cls, key: str) -> tuple["User", Optional[dict]]:
        ...
        select(User).where(User.api_key == key)
        )
        res = res.scalar()        
        if res is None:
...
class Tweet(Base):
    ...
    author: Mapped["User"] = relationship(lazy="selectin", uselist=False, back_populates="tweets")
...
class Like(Base):
    ...
    tweets: Mapped[Tweet] = relationship(lazy="selectin", back_populates="like_as_user_tweet_ass")
...
class UserUser(Base):
    ...
    user: Mapped["User"] = relationship(lazy="selectin", uselist=False, foreign_keys=[id])
    follow_to_user: Mapped["User"] = relationship(lazy="selectin", uselist=False, foreign_keys=[follow_to_id])

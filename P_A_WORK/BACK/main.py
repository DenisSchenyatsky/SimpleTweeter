...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        await session.execute(insert(models.UserUser)
        ...
        await session.execute(insert(models.Tweet)
...
async def user_by_id(user_id: Union[int, str] = 0, 
    ...
    if user_id == 'me':
        ...
        user = res.scalar()
        if user is None:
            return util_func.get_err_dict("not found", f"User with id: {user_id} doesn't exist.")
...
async def end_follow_by_id(api_key: Annotated[str | None, Header()] = None, user_id: int = 0) -> dict:
    """
    <h1>
    ...
    try:
        ...
        if record_uu is None:
            return JSONResponse(
                status_code=404,
                content=util_func.get_err_dict("not found", 
...
async def add_tweet(
    ...
    try:
        ...
        if tweet.tweet_media_ids:
            await session.execute(update(models.Picture)
...
async def delete_tweet(
    ...
    try:    
        res = await session.execute(
            select(models.Tweet)
            .where(and_(models.Tweet.id == tweet_id,
                        models.Tweet.author_id == author_id)
                   )
        )
...
async def get_all_tweets_by_api_key(
    ...
    try:
        ...
        res = await session.execute(
            select(models.Tweet)
            .filter(models.Tweet.author_id == own_id)
        )
...
async def add_like_to_tweet(
    ...
    if tweet is None:
        ...
        if tweet is None:         
            return util_func.get_err_dict("not found", f"Tweet with id: {tweet_id} doesn't exist.")
    try:
...
async def delete_like_from_tweet(
    ...
    if like_record is None:
        return util_func.get_err_dict("not found", 
...
async def get_image_from_form(
    ...
    """
    <h2>
    Загрузка картинки file для твита. </h1> с tweed_id = None.
    ...
    try:
        ...
        # new_picture = models.Picture(file_path=file_path, tweet_id=0)
        # session.add(new_picture)  
        
        res = await session.execute(insert(models.Picture)
